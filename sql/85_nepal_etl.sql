/*********************************************************************
	Loads Nepal data into ME database
...
DROP TABLE IF EXISTS nepal_raw;
DROP TABLE IF EXISTS nepal_updated;
DROP TABLE IF EXISTS nepal_geography;
-- TRUNCATE TABLE report_indicator;
-- TRUNCATE TABLE report_organization;
-- TRUNCATE TABLE report_site;

CREATE TABLE nepal_raw
(
...
COPY nepal_raw (implementing_mechanism, indicator, im_number, baseline_year, baseline_value,
target_2012,
target_2013,
target_2014,
target_2015,
...
admin1,
admin2,
measure) FROM './ftfms/clean_nepal_12_18.csv'
WITH DELIMITER ',' CSV HEADER;


-- create nepal_updated
...
-- update nepal_updated
UPDATE nepal_updated
SET locations = 'Nepal'
WHERE locations is null;


-- update nepal_updated for null measures
UPDATE nepal_updated
SET measure = 'null'
WHERE measure is null;


-- create table nepal_geography
-- table of unique geographies
SELECT * INTO nepal_geography
FROM (
    ...
    from (
        ...
        from (
            ...
            regexp_split_to_table(nepal_updated.locations, E'; ') AS locations -- split into multiple rows
            FROM nepal_updated
...
INSERT INTO district (title, country_id)
	...
	FROM nepal_geography
	JOIN country ON ( country.title = nepal_geography.admin0)
...
(SELECT DISTINCT implementing_mechanism, 1 FROM nepal_updated);
...
INSERT INTO report_organization (organization_id, report_id)
    ...
    JOIN organization ON (organization.title = nepal_updated.prime_partner)
    JOIN report ON (report.title = implementing_mechanism)
    GROUP BY 1,2
    ORDER BY 1 asc;
