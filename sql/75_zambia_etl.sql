...
COPY zambia_raw (implementing_mechanism, indicator, im_number, baseline_year, baseline_value,
target_2012,
target_2013,
target_2014,
target_2015,
...
admin1,
admin2,
measure) FROM './ftfms/zambia.csv'
WITH DELIMITER ',' CSV HEADER;


-- create zambia_updated
...
-- update zambia_updated
UPDATE zambia_updated
SET locations = 'Zambia'
WHERE locations is null;


-- update zambia_updated for null measures
UPDATE zambia_updated
SET measure = 'null'
WHERE measure is null;

-- create table zambia_geography
-- table of unique geographies
SELECT * INTO zambia_geography
FROM (
    ...
    from (
        ...
        from (
            ...
            regexp_split_to_table(zambia_updated.locations, E'; ') AS locations -- split into multiple rows
            FROM zambia_updated
...
INSERT INTO district (title, country_id)
	...
	FROM zambia_geography
	JOIN country ON ( country.title = zambia_geography.admin0)
...
(SELECT DISTINCT implementing_mechanism, 1 FROM zambia_updated);
...
INSERT INTO report_organization (organization_id, report_id)
    ...
    JOIN organization ON (organization.title = zambia_updated.prime_partner)
    JOIN report ON (report.title = implementing_mechanism)
    GROUP BY 1,2
    ORDER BY 1 asc;
