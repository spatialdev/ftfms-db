...
admin1,
admin2,
measure) FROM '/Users/sarahbindman/Documents/CIAT/FTF_DATA/clean_data/clean_ethiopia_12_16.csv'
WITH DELIMITER ',' CSV HEADER;


-- create ethiopia_updated
-- admin level 0 data is populated
-- copy into new table
SELECT *
INTO ethiopia_updated
FROM ethiopia_raw;

-- update ethiopia_updated
UPDATE ethiopia_updated
SET locations = 'Ethiopia'
...
FROM (
    ...
    from (
        ...
        from (
            ...
            regexp_split_to_table(ethiopia_updated.locations, E'; ') AS locations -- split into multiple rows
            FROM ethiopia_updated
...
(SELECT DISTINCT implementing_mechanism, 1 FROM ethiopia_updated);
...
INSERT INTO report_organization (organization_id, report_id)
    ...
    JOIN organization ON (organization.title = ethiopia_updated.prime_partner)
    JOIN report ON (report.title = implementing_mechanism)
    GROUP BY 1,2
    ORDER BY 1 asc;
