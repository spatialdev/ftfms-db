/*********************************************************************
	Loads Tajikistan data into ME database
...
DROP TABLE IF EXISTS tajikistan_raw;
DROP TABLE IF EXISTS tajikistan_updated;
DROP TABLE IF EXISTS tajikistan_geography;
-- TRUNCATE TABLE report_indicator;
-- TRUNCATE TABLE report_organization;
-- TRUNCATE TABLE report_site;

CREATE TABLE tajikistan_raw
(
...
COPY tajikistan_raw (implementing_mechanism, indicator, im_number, baseline_year, baseline_value,
target_2012,
target_2013,
target_2014,
target_2015,
...
admin1,
admin2,
measure) FROM './ftfms/tajikistan.csv'
WITH DELIMITER ',' CSV HEADER;


-- create tajikistan_updated
-- admin level 0 data is populated
-- copy into new table
SELECT *
INTO tajikistan_updated
FROM tajikistan_raw;

-- update tajikistan_updated
UPDATE tajikistan_updated
SET locations = 'Tajikistan'
WHERE locations is null;

-- update tajikistan_updated for null measures
UPDATE tajikistan_updated
SET measure = 'null'
WHERE measure is null;

-- create table tajikistan_geography
-- table of unique geographies
SELECT * INTO tajikistan_geography
FROM (
    ...
    from (
        ...
        from (
            ...
            regexp_split_to_table(tajikistan_updated.locations, E'; ') AS locations -- split into multiple rows
            FROM tajikistan_updated
...
INSERT INTO district (title, country_id)
	...
	FROM tajikistan_geography
	JOIN country ON ( country.title = tajikistan_geography.admin0)
...
INSERT INTO site (district_id, title)
	...
	FROM tajikistan_geography
	JOIN district ON ( district.title = tajikistan_geography.admin1)
	WHERE admin2 IS NOT NULL
	GROUP BY district_id, admin2;


-- geography table is no longer needed
DROP TABLE tajikistan_geography;
...
(SELECT DISTINCT implementing_mechanism, 1 FROM tajikistan_updated);
...
INSERT INTO report_organization (organization_id, report_id)
    ...
    JOIN organization ON (organization.title = tajikistan_updated.prime_partner)
    JOIN report ON (report.title = implementing_mechanism)
    GROUP BY 1,2
    ORDER BY 1 asc;
