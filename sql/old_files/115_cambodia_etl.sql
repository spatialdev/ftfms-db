/*********************************************************************
	Loads Cambodia data into ME database
...
DROP TABLE IF EXISTS cambodia_raw;
DROP TABLE IF EXISTS cambodia_updated;
DROP TABLE IF EXISTS cambodia_geography;
-- TRUNCATE TABLE report_indicator;
-- TRUNCATE TABLE report_organization;
-- TRUNCATE TABLE report_site;

CREATE TABLE cambodia_raw
(
...
COPY cambodia_raw (implementing_mechanism, indicator, im_number, baseline_year, baseline_value,
target_2012,
target_2013,
target_2014,
target_2015,
...
admin1,
admin2,
measure) FROM './ftfms/cambodia.csv'
WITH DELIMITER ',' CSV HEADER;


-- create cambodia_updated
-- admin level 0 data is populated
-- copy into new table
SELECT *
INTO cambodia_updated
FROM cambodia_raw;

-- update cambodia_updated
UPDATE cambodia_updated
SET locations = 'Cambodia'
WHERE locations is null;

-- update cambodia_updated for null measures
UPDATE cambodia_updated
SET measure = 'null'
WHERE measure is null;

-- create table cambodia_geography
-- table of unique geographies
SELECT * INTO cambodia_geography
FROM (
    ...
    from (
        ...
        from (
            ...
            regexp_split_to_table(cambodia_updated.locations, E'; ') AS locations -- split into multiple rows
            FROM cambodia_updated
...
INSERT INTO district (title, country_id)
	...
	FROM cambodia_geography
	JOIN country ON ( country.title = cambodia_geography.admin0)
...
INSERT INTO site (district_id, title)
	...
	FROM cambodia_geography
	JOIN district ON ( district.title = cambodia_geography.admin1)
	WHERE admin2 IS NOT NULL
	GROUP BY district_id, admin2;


-- geography table is no longer needed
DROP TABLE cambodia_geography;
...
(SELECT DISTINCT implementing_mechanism, 1 FROM cambodia_updated);
...
INSERT INTO report_organization (organization_id, report_id)
    ...
    JOIN organization ON (organization.title = cambodia_updated.prime_partner)
    JOIN report ON (report.title = implementing_mechanism)
    GROUP BY 1,2
    ORDER BY 1 asc;
