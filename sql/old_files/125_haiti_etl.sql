/*********************************************************************
	Loads Haiti data into ME database
...
DROP TABLE IF EXISTS haiti_raw;
DROP TABLE IF EXISTS haiti_updated;
DROP TABLE IF EXISTS haiti_geography;
-- TRUNCATE TABLE report_indicator;
-- TRUNCATE TABLE report_organization;
-- TRUNCATE TABLE report_site;

CREATE TABLE haiti_raw
(
...
COPY haiti_raw (implementing_mechanism, indicator, im_number, baseline_year, baseline_value,
target_2012,
target_2013,
target_2014,
target_2015,
...
admin1,
admin2,
measure) FROM './ftfms/haiti.csv'
WITH DELIMITER ',' CSV HEADER;


-- create haiti_updated
...
-- update haiti_updated
UPDATE haiti_updated
SET locations = 'Haiti'
WHERE locations is null;

-- update haiti_updated for null measures
UPDATE haiti_updated
SET measure = 'null'
WHERE measure is null;

-- create table haiti_geography
-- table of unique geographies
SELECT * INTO haiti_geography
FROM (
    ...
    from (
        ...
        from (
            ...
            regexp_split_to_table(haiti_updated.locations, E'; ') AS locations -- split into multiple rows
            FROM haiti_updated
...
INSERT INTO district (title, country_id)
	...
	FROM haiti_geography
	JOIN country ON ( country.title = haiti_geography.admin0)
...
(SELECT DISTINCT implementing_mechanism, 1 FROM haiti_updated);
...
INSERT INTO report_organization (organization_id, report_id)
    ...
    JOIN organization ON (organization.title = haiti_updated.prime_partner)
    JOIN report ON (report.title = implementing_mechanism)
    GROUP BY 1,2
    ORDER BY 1 asc;
