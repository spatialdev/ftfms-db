...
COPY liberia_raw (implementing_mechanism, indicator, im_number, baseline_year, baseline_value,
target_2012,
target_2013,
target_2014,
target_2015,
...
admin1,
admin2,
measure) FROM './ftfms/liberia.csv'
WITH DELIMITER ',' CSV HEADER;


-- create liberia_updated
-- admin level 0 data is populated
-- copy into new table
SELECT *
INTO liberia_updated
FROM liberia_raw;

-- update liberia_updated
UPDATE liberia_updated
SET locations = 'Liberia'
WHERE locations is null;

-- update liberia_updated for null measures
UPDATE liberia_updated
SET measure = 'null'
WHERE measure is null;

-- create table liberia_geography
-- table of unique geographies
SELECT * INTO liberia_geography
FROM (
    ...
    from (
        ...
        from (
            ...
            regexp_split_to_table(liberia_updated.locations, E'; ') AS locations -- split into multiple rows
            FROM liberia_updated
...
INSERT INTO district (title, country_id)
	...
	FROM liberia_geography
	JOIN country ON ( country.title = liberia_geography.admin0)
...
(SELECT DISTINCT implementing_mechanism, 1 FROM liberia_updated);
...
INSERT INTO report_organization (organization_id, report_id)
    ...
    JOIN organization ON (organization.title = liberia_updated.prime_partner)
    JOIN report ON (report.title = implementing_mechanism)
    GROUP BY 1,2
    ORDER BY 1 asc;
