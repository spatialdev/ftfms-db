...
COPY bangladesh_raw (implementing_mechanism, indicator, baseline_year, baseline_value,
target_2012,
target_2013,
target_2014,
target_2015,
...
admin1,
admin2,
measure) FROM './ftfms/bangladesh.csv'
WITH DELIMITER ',' CSV HEADER;


-- create bangladesh_updated
-- admin level 0 data is populated
-- copy into new table
SELECT *
INTO bangladesh_updated
FROM bangladesh_raw;

-- update bangladesh_updated
UPDATE bangladesh_updated
SET locations = 'Bangladesh'
WHERE locations is null;


-- update bangladesh_updated for null measures
UPDATE bangladesh_updated
SET measure = 'null'
WHERE measure is null;


-- create table bangladesh_geography
-- table of unique geographies
SELECT * INTO bangladesh_geography
FROM (
    ...
    from (
        ...
        from (
            ...
            regexp_split_to_table(bangladesh_updated.locations, E'; ') AS locations -- split into multiple rows
...
INSERT INTO district (title, country_id)
	...
	FROM bangladesh_geography
	JOIN country ON ( country.title = bangladesh_geography.admin0)
...
INSERT INTO site (district_id, title)
	...
	FROM bangladesh_geography
	JOIN district ON ( district.title = bangladesh_geography.admin1)
...
(SELECT DISTINCT implementing_mechanism, 1 FROM bangladesh_updated);
