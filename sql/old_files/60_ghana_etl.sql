...
COPY ghana_raw (implementing_mechanism, indicator, im_number, baseline_year, baseline_value,
target_2012,
target_2013,
target_2014,
target_2015,
...
admin1,
admin2,
measure) FROM './ftfms/ghana.csv'
WITH DELIMITER ',' CSV HEADER;


-- create ghana_updated
...
-- update ghana_updated
UPDATE ghana_updated
SET locations = 'Ghana'
WHERE locations is null;

-- update ghana_updated for null measures
UPDATE ghana_updated
SET measure = 'null'
WHERE measure is null;

-- create table ghana_geography
-- table of unique geographies
SELECT * INTO ghana_geography
FROM (
    ...
    from (
        ...
        from (
            ...
            regexp_split_to_table(ghana_updated.locations, E'; ') AS locations -- split into multiple rows
            FROM ghana_updated
...
INSERT INTO district (title, country_id)
	...
	FROM ghana_geography
	JOIN country ON ( country.title = ghana_geography.admin0)
...
(SELECT DISTINCT implementing_mechanism, 1 FROM ghana_updated);
...
INSERT INTO report_organization (organization_id, report_id)
    ...
    JOIN organization ON (organization.title = ghana_updated.prime_partner)
    JOIN report ON (report.title = implementing_mechanism)
    GROUP BY 1,2
    ORDER BY 1 asc;
