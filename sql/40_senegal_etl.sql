/*********************************************************************
	Loads Senegal data into ME database
...
COPY senegal_raw (implementing_mechanism, indicator, im_number, baseline_year, baseline_value,
target_2012,
target_2013,
target_2014,
target_2015,
...
admin1,
admin2,
measure) FROM './ftfms/clean_senegal_12_18.csv'
WITH DELIMITER ',' CSV HEADER;


-- create senegal_updated
-- admin level 0 data is populated
-- copy into new table
SELECT *
INTO senegal_updated
FROM senegal_raw;
...
-- update senegal_updated for null measures
UPDATE senegal_updated
SET measure = 'null'
WHERE measure is null;


-- create table senegal_geography
-- table of unique geographies
SELECT * INTO senegal_geography
FROM (
    ...
    from (
        ...
        from (
            ...
            regexp_split_to_table(senegal_updated.locations, E'; ') AS locations -- split into multiple rows
            FROM senegal_updated
...
INSERT INTO district (title, country_id)
	...
	FROM senegal_geography
	JOIN country ON ( country.title = senegal_geography.admin0)
...
(SELECT DISTINCT implementing_mechanism, 1 FROM senegal_updated);
...
INSERT INTO report_organization (organization_id, report_id)
    ...
    JOIN organization ON (organization.title = senegal_updated.prime_partner)
    JOIN report ON (report.title = implementing_mechanism)
    GROUP BY 1,2
    ORDER BY 1 asc;
