/*********************************************************************
	Loads Rwanda data into ME database
...
DROP TABLE IF EXISTS rwanda_raw;
DROP TABLE IF EXISTS rwanda_updated;
DROP TABLE IF EXISTS rwanda_geography;
-- TRUNCATE TABLE report_indicator;
-- TRUNCATE TABLE report_organization;
-- TRUNCATE TABLE report_site;

CREATE TABLE rwanda_raw
(
...
COPY rwanda_raw (implementing_mechanism, indicator, im_number, baseline_year, baseline_value,
target_2012,
target_2013,
target_2014,
target_2015,
...
admin1,
admin2,
measure) FROM './ftfms/clean_rwanda_12_18.csv'
WITH DELIMITER ',' CSV HEADER;


-- create rwanda_updated
...
-- update rwanda_updated
UPDATE rwanda_updated
SET locations = 'Rwanda'
WHERE locations is null;

-- update rwanda_updated for null measures
UPDATE rwanda_updated
SET measure = 'null'
WHERE measure is null;


-- create table rwanda_geography
-- table of unique geographies
SELECT * INTO rwanda_geography
FROM (
    ...
    from (
        ...
        from (
            ...
            regexp_split_to_table(rwanda_updated.locations, E'; ') AS locations -- split into multiple rows
            FROM rwanda_updated
...
INSERT INTO district (title, country_id)
	...
	FROM rwanda_geography
	JOIN country ON ( country.title = rwanda_geography.admin0)
...
(SELECT DISTINCT implementing_mechanism, 1 FROM rwanda_updated);
...
INSERT INTO report_organization (organization_id, report_id)
    ...
    JOIN organization ON (organization.title = rwanda_updated.prime_partner)
    JOIN report ON (report.title = implementing_mechanism)
    GROUP BY 1,2
    ORDER BY 1 asc;
