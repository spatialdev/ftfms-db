/*********************************************************************
	Loads Mozambique data into ME database
...
DROP TABLE IF EXISTS mozambique_raw;
DROP TABLE IF EXISTS mozambique_updated;
DROP TABLE IF EXISTS mozambique_geography;
-- TRUNCATE TABLE report_indicator;
-- TRUNCATE TABLE report_organization;
-- TRUNCATE TABLE report_site;

CREATE TABLE mozambique_raw
(
...
COPY mozambique_raw (implementing_mechanism, indicator, im_number, baseline_year, baseline_value,
target_2012,
target_2013,
target_2014,
target_2015,
...
admin1,
admin2,
measure) FROM './ftfms/clean_mozambique_12_18.csv'
WITH DELIMITER ',' CSV HEADER;


-- create mozambique_updated
-- admin level 0 data is populated
-- copy into new table
SELECT *
INTO mozambique_updated
FROM mozambique_raw;

-- update mozambique_updated
UPDATE mozambique_updated
SET locations = 'Mozambique'
WHERE locations is null;

-- update mozambique_updated for null measures
UPDATE mozambique_updated
SET measure = 'null'
WHERE measure is null;

-- create table mozambique_geography
-- table of unique geographies
SELECT * INTO mozambique_geography
FROM (
    ...
    from (
        ...
        from (
            ...
            regexp_split_to_table(mozambique_updated.locations, E'; ') AS locations -- split into multiple rows
            FROM mozambique_updated
...
INSERT INTO district (title, country_id)
	...
	FROM mozambique_geography
	JOIN country ON ( country.title = mozambique_geography.admin0)
...
INSERT INTO site (district_id, title)
	...
	FROM mozambique_geography
	JOIN district ON ( district.title = mozambique_geography.admin1)
	WHERE admin2 IS NOT NULL
	GROUP BY district_id, admin2;


-- geography table is no longer needed
DROP TABLE mozambique_geography;
...
(SELECT DISTINCT implementing_mechanism, 1 FROM mozambique_updated);
...
INSERT INTO report_organization (organization_id, report_id)
    ...
    JOIN organization ON (organization.title = mozambique_updated.prime_partner)
    JOIN report ON (report.title = implementing_mechanism)
    GROUP BY 1,2
    ORDER BY 1 asc;
