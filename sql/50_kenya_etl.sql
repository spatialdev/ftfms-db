...
COPY kenya_raw (implementing_mechanism, indicator, im_number, baseline_year, baseline_value,
target_2012,
target_2013,
target_2014,
target_2015,
...
admin1,
admin2,
measure) FROM './ftfms/kenya.csv'
WITH DELIMITER ',' CSV HEADER;


-- create kenya_updated
...
-- update kenya_updated for null measures
UPDATE kenya_updated
SET measure = 'null'
...
-- create table kenya_geography
-- table of unique geographies
SELECT * INTO kenya_geography
FROM (
    ...
    from (
        ...
        from (
            ...
            regexp_split_to_table(kenya_updated.locations, E'; ') AS locations -- split into multiple rows
            FROM kenya_updated
...
INSERT INTO district (title, country_id)
	...
	FROM kenya_geography
	JOIN country ON ( country.title = kenya_geography.admin0)
...
(SELECT DISTINCT implementing_mechanism, 1 FROM kenya_updated);
...
INSERT INTO report_organization (organization_id, report_id)
    ...
    JOIN organization ON (organization.title = kenya_updated.prime_partner)
    JOIN report ON (report.title = implementing_mechanism)
    GROUP BY 1,2
    ORDER BY 1 asc;
