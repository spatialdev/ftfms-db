/*********************************************************************
	Loads Honduras data into ME database
...
DROP TABLE IF EXISTS honduras_raw;
DROP TABLE IF EXISTS honduras_updated;
DROP TABLE IF EXISTS honduras_geography;
-- TRUNCATE TABLE report_indicator;
-- TRUNCATE TABLE report_organization;
-- TRUNCATE TABLE report_site;

CREATE TABLE honduras_raw
(
...
COPY honduras_raw (implementing_mechanism, indicator, im_number, baseline_year, baseline_value,
target_2012,
target_2013,
target_2014,
target_2015,
...
admin1,
admin2,
measure) FROM './ftfms/honduras.csv'
WITH DELIMITER ',' CSV HEADER;


-- create honduras_updated
-- admin level 0 data is populated
-- copy into new table
SELECT *
INTO honduras_updated
FROM honduras_raw;

-- update honduras_updated
UPDATE honduras_updated
SET locations = 'Honduras'
WHERE locations is null;

-- update honduras_updated for null measures
UPDATE honduras_updated
SET measure = 'null'
WHERE measure is null;

-- create table honduras_geography
-- table of unique geographies
SELECT * INTO honduras_geography
FROM (
    ...
    from (
        ...
        from (
            ...
            regexp_split_to_table(honduras_updated.locations, E'; ') AS locations -- split into multiple rows
            FROM honduras_updated
...
INSERT INTO district (title, country_id)
	...
	FROM honduras_geography
	JOIN country ON ( country.title = honduras_geography.admin0)
...
INSERT INTO site (district_id, title)
	...
	FROM honduras_geography
	JOIN district ON ( district.title = honduras_geography.admin1)
	WHERE admin2 IS NOT NULL
	GROUP BY district_id, admin2;


-- geography table is no longer needed
DROP TABLE honduras_geography;
...
(SELECT DISTINCT implementing_mechanism, 1 FROM honduras_updated);
...
INSERT INTO report_organization (organization_id, report_id)
    ...
    JOIN organization ON (organization.title = honduras_updated.prime_partner)
    JOIN report ON (report.title = implementing_mechanism)
    GROUP BY 1,2
    ORDER BY 1 asc;
