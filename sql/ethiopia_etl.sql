...
FROM (
    ...
    from (
        ...
        from (
            ...
            regexp_split_to_table(ethiopia_raw.locations, E'; ') AS locations -- split into multiple rows
            FROM ethiopia_raw
...
(SELECT DISTINCT implementing_mechanism, 1 FROM ethiopia_raw);
...
INSERT INTO report_organization (organization_id, report_id)
    ...
    JOIN organization ON (organization.title = ethiopia_raw.prime_partner)
    JOIN report ON (report.title = implementing_mechanism)
    GROUP BY 1,2
    ORDER BY 1 asc;
