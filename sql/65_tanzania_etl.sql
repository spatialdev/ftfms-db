/*********************************************************************
	Loads Tanzania data into ME database
...
DROP TABLE IF EXISTS tanzania_raw;
DROP TABLE IF EXISTS tanzania_updated;
DROP TABLE IF EXISTS tanzania_geography;
-- TRUNCATE TABLE report_indicator;
-- TRUNCATE TABLE report_organization;
-- TRUNCATE TABLE report_site;

CREATE TABLE tanzania_raw
(
...
COPY tanzania_raw (implementing_mechanism, indicator, im_number, baseline_year, baseline_value,
target_2012,
target_2013,
target_2014,
target_2015,
...
admin1,
admin2,
measure) FROM './ftfms/clean_tanzania_12_18.csv'
WITH DELIMITER ',' CSV HEADER;


-- create tanzania_updated
-- admin level 0 data is populated
-- copy into new table
SELECT *
INTO tanzania_updated
FROM tanzania_raw;

-- update tanzania_updated
UPDATE tanzania_updated
SET locations = 'Tanzania'
WHERE locations is null;

-- update tanzania_updated for null measures
UPDATE tanzania_updated
SET measure = 'null'
WHERE measure is null;

-- create table tanzania_geography
-- table of unique geographies
SELECT * INTO tanzania_geography
FROM (
    ...
    from (
        ...
        from (
            ...
            regexp_split_to_table(tanzania_updated.locations, E'; ') AS locations -- split into multiple rows
            FROM tanzania_updated
...
INSERT INTO district (title, country_id)
	...
	FROM tanzania_geography
	JOIN country ON ( country.title = tanzania_geography.admin0)
...
INSERT INTO site (district_id, title)
	...
	FROM tanzania_geography
	JOIN district ON ( district.title = tanzania_geography.admin1)
	WHERE admin2 IS NOT NULL
	GROUP BY district_id, admin2;


-- geography table is no longer needed
DROP TABLE tanzania_geography;
...
(SELECT DISTINCT implementing_mechanism, 1 FROM tanzania_updated);
...
INSERT INTO report_organization (organization_id, report_id)
    ...
    JOIN organization ON (organization.title = tanzania_updated.prime_partner)
    JOIN report ON (report.title = implementing_mechanism)
    GROUP BY 1,2
    ORDER BY 1 asc;
